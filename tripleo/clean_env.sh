...
if [[ "$ENVIRONMENT_OS" == 'rhel' ]]; then
  rhel_unregister_system $poolname\undercloud-$NUM.qcow2 || true
  rhel_unregister_system $poolname\undercloud-$NUM-cert-test.qcow2 || true
fi
...
for vol in `virsh vol-list $poolname | awk "/overcloud-$NUM-/ {print \$1}"` ; do
  if [[ "$ENVIRONMENT_OS" == 'rhel' ]]; then
    rhel_unregister_system $poolname\$vol || true
  fi
