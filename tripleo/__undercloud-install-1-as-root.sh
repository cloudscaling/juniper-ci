...
if [[ 'newton|ocata' =~ $OPENSTACK_VERSION  ]] ; then
  ...
  if [[ "$DPDK" != 'off' ]] ; then
    ...
    curl -o ${repo_dir}/python-docker-pycreds-1.10.6-4.el7.noarch.rpm http://mirror.centos.org/centos/7/extras/x86_64/Packages/python-docker-pycreds-1.10.6-4.el7.noarch.rpm
    # ---
    update_contrail_repo='yes'
  fi
