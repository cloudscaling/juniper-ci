...
if [[ -z "$image" ]]; then
  docker pull centos
  docker run -i --name cprep-$JOB_RND --entrypoint /bin/bash centos -c "yum install -y epel-release && yum install -y ansible python-ipaddress git python-pip sudo vim gcc python-devel && pip install pip --upgrade && pip install pycrypto oslo_utils oslo_config"
